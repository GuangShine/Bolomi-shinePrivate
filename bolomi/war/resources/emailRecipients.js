...
mango.erecip.EmailRecipientssn = function(prefix, testEmailMessage, mailingLists, users) {
    this.prefix = prefix;
    this.testEmailMessage = testEmailMessage;
    this.nextAddressId = 1;
    this.mailingLists = mailingLists;
    ...
    this.write = function(node, varName, id, label) {
        ...
        td.className = "formLabelRequired";
        var cnt = label +'<br/>';
        cnt += '<img id="'+ this.prefix +'SendTestImgsn" src="images/email_go.png" class="ptr"';
        cnt += '        onclick="'+ varName +'.sendTestEmail()" title="'+ mango.i18n["common.sendTestEmail"] +'"/>';
        td.innerHTML = cnt;
        ...
        cnt += '    <td class="formLabel">'+ mango.i18n["js.email.addMailingList"] +'</td>';
        cnt += '    <td>';
        cnt += '      <select id="'+ this.prefix +'MailingListssn"></select>';
        cnt += '      <img src="images/add.png" class="ptr" onclick="'+ varName +'.addMailingList()"/>';
        cnt += '    </td>';
        cnt += '  </tr>';
        cnt += '  <tr>';
        cnt += '    <td class="formLabel">'+ mango.i18n["js.email.addUser"] +'</td>';
        cnt += '    <td>';
        cnt += '      <select id="'+ this.prefix +'Userssn"></select>';
        cnt += '      <img src="images/add.png" class="ptr" onclick="'+ varName +'.addUser()"/>';
        cnt += '    </td>';
        cnt += '  </tr>';
        cnt += '  <tr>';
        cnt += '    <td class="formLabel">'+ mango.i18n["js.email.addAddress"] +'</td>';
        cnt += '    <td>';
        cnt += '      <input id="'+ this.prefix +'Addresssn" type="text"/>';
        cnt += '      <img src="images/add.png" class="ptr" onclick="'+ varName +'.addAddress()"/>';
        cnt += '    </td>';
        cnt += '  </tr>';
        cnt += '</table>';
        cnt += '<div class="borderDivPadded">';
        cnt += '  <table width="100%">';
        cnt += '    <tr id="'+ this.prefix +'ListEmptysn"><td colspan="3">'+ mango.i18n["js.email.noRecipients"] +'</td></tr>';
        cnt += '    <tr id="'+ this.prefix +'_TEMPLATE_sn" style="display:none;">';
        cnt += '        <td width="16"><img id="'+ this.prefix +'_TEMPLATE_Imgsn"/></td>';
        cnt += '        <td id="'+ this.prefix +'_TEMPLATE_Descriptionsn"></td>';
        cnt += '        <td width="16"><img src="images/bullet_delete.png" class="ptr"';
        cnt += '                onclick="'+ varName +'.deleteRecipient(getMangoId(this))"/></td>';
        cnt += '    </tr>';
        cnt += '    <tbody id="'+ this.prefix +'Listsn"></tbody>';
        cnt += '  </table>';
        cnt += '</div>';
        cnt += '<span id="'+ this.prefix +'Errorsn" class="formError"></span>';
        td.innerHTML = cnt;
    }
    
    this.repopulateLists = function() {
        dwr.util.removeAllOptions(this.prefix +"MailingListssn");
        dwr.util.addOptions(this.prefix +"MailingListssn", this.mailingLists, "id", "name");
        dwr.util.removeAllOptions(this.prefix +"Userssn");
        dwr.util.addOptions(this.prefix +"Userssn", this.users, "id", "username");
    }
    ...
    this.sendTestEmail = function() {
        ...
        if (emailList.length == 0)
            ...
            MiscDwr.sendTestEmail(emailList, this.prefix, this.testEmailMessage, dojo.lang.hitch(this, "sendTestEmailCB"));
            startImageFader($(this.prefix +"SendTestImgsn"));
        }
    }

    this.sendTestEmailCB = function(response) {
        stopImageFader($(this.prefix +"SendTestImgsn"));
        if (response.messages.length > 0)
            this.setErrorMessage(response.messages[0]);
        else
            this.setErrorMessage(mango.i18n["js.email.testSent"]);
    ...
    this.deleteRecipient = function(id) {
        // Delete the visual entry.
        $(this.prefix +"Listsn").removeChild($(this.prefix + id+"sn"));
        this.updateOptions(this.prefix +"MailingListssn", this.mailingLists, this.prefix +"Listsn", "M", "name");
        this.updateOptions(this.prefix +"Userssn", this.users, this.prefix +"Listsn", "U", "username");
        this.checkListEmptyMessage();
    }
    
    this.addMailingList = function(mlid) {
        if (!mlid)
            mlid = $get(this.prefix +"MailingListssn");
        if (!mlid)
            return;
        var ml = this.getMailingList(mlid);
        if (ml) {
            this.addListEntry("M"+ mlid, "images/book.png", ml.name);
            this.updateOptions(this.prefix +"MailingListssn", this.mailingLists, this.prefix +"List", "M", "name");
            this.checkListEmptyMessage();
        }
    }
    ...
    this.addUser = function(uid) {
        if (!uid)
            uid = $get(this.prefix +"Userssn");
        if (!uid)
            return;
        var user = this.getUser(uid);
        if (user) {
            this.addListEntry("U"+ uid, "images/user.png", user.username);
            setUserImg(user.admin, user.disabled, $(this.prefix +"U"+ uid +"Imgsn"));
            this.updateOptions(this.prefix +"Userssn", this.users, this.prefix +"Listsn", "U", "username");
            this.checkListEmptyMessage();
        }
    }
    ...
    this.addAddress = function(addr) {
        if (!addr)
            addr = $get(this.prefix +"Addresssn");
        if (addr == "")
            return;
        this.addListEntry("A"+ this.nextAddressId++, "images/email.png", addr);
        this.checkListEmptyMessage();
    ...
    this.createRecipientArray = function() {
        var recipientList = $(this.prefix +"Listsn");
        var list = new Array();
        var id;
        for (j=0; j<recipientList.childNodes.length; j++) {
            if (recipientList.childNodes[j].mangoId) {
                ...
                if (id.startsWith("M"))
                    ...
                    list[list.length] = {
                            recipientType: 3, // EmailRecipient.TYPE_ADDRESS
                            referenceAddress: $(this.prefix + id +"Descriptionsn").innerHTML};
                else
                    dojo.debug("Unknown recipient mango id: "+ id);
            }
        }
    ...
    this.addListEntry = function(id, imgName, description) {
        createFromTemplate(this.prefix +"_TEMPLATE_sn", id, this.prefix +"Listsn");
        $(this.prefix + id +"Imgsn").src = imgName;
        $(this.prefix + id +"Descriptionsn").innerHTML = description;
    }
    ...
    this.checkListEmptyMessage = function() {
        ...
        display(this.prefix +"ListEmptysn", !found);
    }
    
    this.updateRecipientList = function(recipientList) {
        this.setErrorMessage();
        this.repopulateLists();
        dwr.util.removeAllRows(this.prefix +"Listsn");
        if (!recipientList || recipientList.length == 0)
            this.checkListEmptyMessage();
        else {
            for (var i=0; i<recipientList.length; i++) {
    ...
    this.setErrorMessage = function(msg) {
        showMessage(this.prefix +"Errorsn", msg);
    }
}
