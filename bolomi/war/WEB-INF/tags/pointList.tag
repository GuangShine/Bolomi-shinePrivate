          ...
          <tr>
            ...
            <td align="right">
              <tag:img id="editImg${applicationScope['constants.Common.NEW_ID']}" png="icon_comp_add"
                      onclick="editPoint(${applicationScope['constants.Common.NEW_ID']},null)" />
    ...
    <td valign="top">
      <div id="pointDetails" class="borderDiv marB" style="display: none;">
        ...
        <table>
          ...
          <tr>
          	    <td class="formLabelRequired"><%-- <fmt:message key="dsEdit.head.name"/> --%>key</td>
            <td class="formField">
			     <c:forEach items="${keytype}" var="item" varStatus="status">
			        <label><input name="keytype_point" type="checkbox" value="<c:out value="${item.key_Code}" />"  /><c:out value="${item.key_Name}" /></label>
