...
<tag:page dwr="PublisherEditDwr" onload="initPublisher">
  <script type="text/javascript">
    ...
    function sendFreshotChanged() {
        if ($get("sendFre")) {
            setDisabled("SendFrePeriods", false);
            setDisabled("SendFrePeriodType", false);
        }
        else {
            setDisabled("SendFrePeriods", true);
            setDisabled("SendFrePeriodType", true);
        }
    }
  ...
  <table cellspacing="0" cellpadding="0" border="0">
    <tr>
      <td>
        ...
        ${publisher.type.id}  
        ${applicationScope['constants.PublisherVO.Types.HTTPJsonCtc_SENDER']} 
        <div>
          <c:choose>
            ...
            <c:when test="${publisher.type.id == applicationScope['constants.PublisherVO.Types.HTTPJson_SENDER']}">
              <%@ include file="/WEB-INF/jsp/publisherEdit/editHttpJsonSender.jsp" %>
            </c:when>
             <c:when test="${publisher.type.id == applicationScope['constants.PublisherVO.Types.HTTPJsonAlerm_SENDER']}">
              <%@ include file="/WEB-INF/jsp/publisherEdit/editHttpJsonAlermSender.jsp" %>
            </c:when>
             <c:when test="${publisher.type.id == applicationScope['constants.PublisherVO.Types.HTTPJsonCtc_SENDER']}">
              <%@ include file="/WEB-INF/jsp/publisherEdit/editHttpJsonCtcSender.jsp" %>
            </c:when>
             <c:when test="${publisher.type.id == applicationScope['constants.PublisherVO.Types.HTTPJsonHeartbeatCtc_SENDER']}">
              <%@ include file="/WEB-INF/jsp/publisherEdit/editHttpJsonHeartbeatCtcSender.jsp" %>
            </c:when>
          </c:choose>
        </div>
      </td>
