...
<tag:page dwr="UsersDwr" onload="init">
  <script type="text/javascript">
    ...
    function showUserCB(user) {
        ...
        var permisstr=user.permission;
        var permisstrs= new Array();  
        permisstrs=permisstr.split("|"); 
		if(permisstrs[0]=="1")
			{
				$set("checkboxWatchList",true);
			}
		if(permisstrs[1]=="1")
		{
			$set("checkboxAlarms",true);
		}
		if(permisstrs[2]=="1")
		{
			$set("checkboxEventHandlers",true);
		}
		if(permisstrs[3]=="1")
		{
			$set("checkboxDataSources",true);
		}
		if(permisstrs[4]=="1")
		{
			$set("checkboxUsers",true);
		}
		
		if(permisstrs[5]=="1")
		{
			$set("checkboxPointHierarchy",true);
		}
		if(permisstrs[6]=="1")
		{
			$set("checkboxMailingLists",true);
		}
		if(permisstrs[7]=="1")
		{
			$set("checkboxPublishers",true);
		}
		if(permisstrs[8]=="1")
		{
			$set("checkboxSystemSettings",true);
		}
		
		if(permisstrs[9]=="1")
		{
			$set("checkboxEmport",true);
		}
		if(permisstrs[10]=="1")
		{
			$set("checkboxSql",true);
		}		}
        if (adminUser) {
    ...
    function saveUser() {
        ...
        if (adminUser) {
            ...
            UsersDwr.saveUserAdmin(editingUserId, $get("username"), $get("password"), $get("email"), $get("phone"), 
                    ...
                    dsPermis, dpPermis,permissionstr, saveUserCB);
        }
        else
            UsersDwr.saveUser(editingUserId, $get("password"), $get("email"), $get("phone"),
                    $get("receiveAlarmEmails"), $get("receiveOwnAuditEvents"),permissionstr, saveUserCB);
    }
          ...
          <table>
            ...
            <tbody id="dataSources" style="display:none;">
			 ...
			 <tbody id="permission">
              <tr><td class="horzSeparator" colspan="2"></td></tr>
              <tr>
            	<td class="formLabelRequired"><fmt:message key="header.watchlist"/>
              	<!--<fmt:message key="header.publishers"/><input id="checkboxPublishers" type="checkbox"/>-->
              	</td>
              	<td><input id="checkboxWatchList" type="checkbox"/></td>
              	
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.alarms"/></td>
            	<td><input id="checkboxAlarms" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.eventHandlers"/></td>
            	<td><input id="checkboxEventHandlers" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.dataSources"/></td>
            	<td><input id="checkboxDataSources" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.users"/></td>
            	<td><input id="checkboxUsers" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.pointHierarchy"/></td>
            	<td><input id="checkboxPointHierarchy" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.mailingLists"/></td>
            	<td><input id="checkboxMailingLists" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.publishers"/></td>
            	<td><input id="checkboxPublishers" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.systemSettings"/></td>
            	<td><input id="checkboxSystemSettings" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.emport"/></td>
            	<td><input id="checkboxEmport" type="checkbox"/></td>
            </tr>
            <tr>
            	<td class="formLabelRequired"><fmt:message key="header.sql"/></td>
            	<td><input id="checkboxSql" type="checkbox"/></td>
            </tr>
          </table>
        </div>
      </td>
