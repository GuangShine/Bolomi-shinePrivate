...
--%>
<%@page import="com.serotonin.mango.vo.publish.httpJsonSender.HttpJsonSenderVO"%>
...
<script type="text/javascript">
  ...
  function initCB(response) {
      ...
      for (var i=0; i<list.length; i++)
          allPoints[allPoints.length] = {
                  id: list[i].id, name: list[i].extendedName, enabled: list[i].enabled, type: list[i].dataTypeMessage,xid:list[i].xid};
  ...
  function selectPoint() {
      ...
      for(var item in pointId){
    	  addToSelectedArray(pointId[item], null, true);
      }
/*       
      var pointId = $get("availablePoints");
  ...
  function savePublisherImpl(name, xid, enabled, cacheWarningSize, changesOnly, sendSnapshot, snapshotSendPeriods,
      ...
      PublisherEditDwr.saveHttpJsonHeartBeatCtcSender(name, xid, enabled, points, $get("url"), $get("usePost") == "true", 
    		  staticHeaderList, staticParameterList, cacheWarningSize, changesOnly, $get("raiseResultWarning"),
    		  $get("dateFormat"), sendSnapshot, snapshotSendPeriods, snapshotSendPeriodType, $get("HeartbeatRateType"),
              $get("HeartbeatRate"),savePublisherCB);
  }
  ...
  function httpSendTest() {
      ...
      PublisherEditDwr.httpJsonCtcSenderTest($get("url"), $get("usePost") == "true", staticHeaderList, staticParameterList,
    		  httpSendTestCB);
  }
  ...
  function httpSendTestUpdate() {
      PublisherEditDwr.httpJsonCtcSenderTestUpdate(httpSendTestUpdateCB);
  }
        ...
        <table>
          <tr>
            <td colspan="2" class="smallTitle"><fmt:message key="publisherEdit.httpSender.props"/> <tag:help id="httpSenderPublishing"/></td>
          	...
          	<tr>
          		<td class="formLabelRequired"><fmt:message key="publisherEdit.httpJsonCtcHeartbeatSender.sendHearBeatRate"/></td>
          		<td>
          		  <input type="text" id="HeartbeatRate" value="${publisher.heartBeatValue}" class="formShort"/>
                <sst:select id="HeartbeatRateType" value="${publisher.heartBeatType}">
...
<%--           <tr>
              <td class="formLabelRequired">sendfree</td>
              <td class="formField"><sst:checkbox id="sendFre" onclick="sendFreshotChanged()"
                      selectedValue="${publisher.sendFreShot}"/></td>
            </tr>
            
            <tr>
              <td class="formLabelRequired">sendFreTime</td>
              <td class="formField">
                <input type="text" id="SendFrePeriods" value="${publisher.sendValue}" class="formShort"/>
                <sst:select id="SendFrePeriodType" value="${publisher.sendType}">
                  <tag:timePeriodOptions sst="true" ms="true" s="true" min="true" h="true"/>
                </sst:select>
          ...
          <tr>
            ...
            <td class="formField">
              <sst:select id="dateFormat" value="${publisher.dateFormat}">
                <sst:option value="<%= Integer.toString(HttpJsonSenderVO.DATE_FORMAT_BASIC) %>"><fmt:message key="publisherEdit.httpSender.dateFormat.basic"/></sst:option>
                <sst:option value="<%= Integer.toString(HttpJsonSenderVO.DATE_FORMAT_TZ) %>"><fmt:message key="publisherEdit.httpSender.dateFormat.tz"/></sst:option>
                <sst:option value="<%= Integer.toString(HttpJsonSenderVO.DATE_FORMAT_UTC) %>"><fmt:message key="publisherEdit.httpSender.dateFormat.utc"/></sst:option>
              </sst:select>
            </td>
          </tr>
        </table>
  ...
  <div class="borderDiv">
    <table width="100%">
      <tr>
        ...
        <td align="right">
          <select  multiple="multiple" size="5" id="availablePoints"></select>
