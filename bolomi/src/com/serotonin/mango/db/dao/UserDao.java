...
public class UserDao extends BaseDao {
    private static final String USER_SELECT = "select id, username, password, email, phone, admin, disabled, selectedWatchList, homeUrl, lastLogin, "
            + "  receiveAlarmEmails, receiveOwnAuditEvents ,permission " + "from users ";
    ...
    class UserRowMapper implements GenericRowMapper<User> {
        public User mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            user.setPermission(rs.getString(++i));
            return user;
        }
    }
    ...
    private static final String USER_INSERT = "insert into users ("
            + "  username, password, email, phone, admin, disabled, homeUrl, receiveAlarmEmails, receiveOwnAuditEvents,permission) "
            + "values (?,?,?,?,?,?,?,?,?,?)";
    ...
    void insertUser(User user) {
        int id = doInsert(
                ...
                new Object[] { user.getUsername(), user.getPassword(), user.getEmail(), user.getPhone(),
                        ...
                        user.getReceiveAlarmEmails(), boolToChar(user.isReceiveOwnAuditEvents()),user.getPermission() }, new int[] {
    ...
    private static final String USER_UPDATE = "update users set "
            ...
            + "  receiveOwnAuditEvents=? , permission=? " + "where id=?";
    ...
    void updateUser(User user) {
        ejt.update(
                ...
                new Object[] { user.getUsername(), user.getPassword(), user.getEmail(), user.getPhone(),
                        boolToChar(user.isAdmin()), boolToChar(user.isDisabled()), user.getHomeUrl(),
                        user.getReceiveAlarmEmails(), boolToChar(user.isReceiveOwnAuditEvents()),user.getPermission(), user.getId() },
    ...
    public void deleteUser(final int userId) {
        getTransactionTemplate().execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                ...
                ejt.update("update events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime set ackUserId=null, alternateAckSource="
