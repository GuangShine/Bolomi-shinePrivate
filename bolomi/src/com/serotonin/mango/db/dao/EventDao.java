...
public class EventDao extends BaseDao {
    ...
    public void saveEvent(EventInstance event) {
        if (event.getId().equals(String.valueOf(Common.NEW_ID)))
            insertEvent(event);
        else
            updateEvent(event);
    }

    private static final String EVENT_INSERT = "insert into events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime (id,typeId, typeRef1, typeRef2, activeTs, rtnApplicable, rtnTs, rtnCause, "
            + "  alarmLevel, message, ackTs) " + "values (?,?,?,?,?,?,?,?,?,?,?)";
    private static final int[] EVENT_INSERT_TYPES = { Types.VARCHAR,Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.BIGINT,
            Types.CHAR, Types.BIGINT, Types.INTEGER, Types.INTEGER, Types.VARCHAR, Types.BIGINT };


    public void InsertEventInfo(EventInstance event)
    {
    	int[] EVENT_INFO_INSERT_TYPES = { Types.VARCHAR,Types.VARCHAR, Types.VARCHAR };
    	 Object[] args = new Object[3];
		    args[0]=  event.getId();
		    args[1] = Common.getMessage(event.getMessage().getKey(), event.getMessage().getArgs());
		    args[2] = event.getActiveTimestamp();
		    ejt.update("delete from eventInfo where id=?",new Object[] { event.getId()});
    	ejt.update("insert into eventInfo (id,message,ts) values(?,?,?)", args, EVENT_INFO_INSERT_TYPES);
    }
    public void updateEventInfo(EventInstance event)
    {
    	int[] EVENT_INFO_INSERT_TYPES = { Types.VARCHAR,Types.VARCHAR, Types.VARCHAR };
   	 Object[] args = new Object[3];
		    args[0]=  Common.getMessage(event.getMessage().getKey(), event.getMessage().getArgs());
		    args[1] = event.getActiveTimestamp();
		    args[2] = event.getId();
		    ejt.update("update eventinfo set message=?,ts=? where id=?", args, EVENT_INFO_INSERT_TYPES);
    }
    private void insertEvent(EventInstance event) {
        EventType type = event.getEventType();
        Calendar date=Calendar.getInstance();
    	String valueid= Common.getEnvironmentProfile().getString("db.mangocode")+type.getEventSourceId()+type.getDataSourceId()+type.getDataPointId()+new Date().getTime()+System.currentTimeMillis(); 
       
        Object[] args = new Object[11];
        args[0]= valueid;
        args[1] = type.getEventSourceId();
        ...
        args[9] = event.getMessage().serialize();
        if (!event.isAlarm()) {
            event.setAcknowledgedTimestamp(event.getActiveTimestamp());
            args[10] = event.getAcknowledgedTimestamp();
        }
        ejt.update(EVENT_INSERT, args, EVENT_INSERT_TYPES);
        /*doInsertLong(EVENT_INSERT, args, EVENT_INSERT_TYPES);*/
        event.setId(valueid);
        event.setEventComments(new LinkedList<UserComment>());
    }

    private static final String EVENT_UPDATE = "update events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime set rtnTs=?, rtnCause=? where id=?";
    ...
    private static final String EVENT_ACK = "update events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime set ackTs=?, ackUserId=?, alternateAckSource=? where id=? and ackTs is null";
    private static final String USER_EVENT_ACK = "update userEvents set silenced=? where eventId=?";

    public void ackEvent(String eventId, long time, int userId, int alternateAckSource) {
        // Ack the event
    ...
    public void insertUserEvents(final String eventId, final List<Integer> userIds, final boolean alarm) {
    ...
    private static final String BASIC_EVENT_SELECT = "select e.id, e.typeId, e.typeRef1, e.typeRef2, e.activeTs, e.rtnApplicable, e.rtnTs, e.rtnCause, "
            ...
            + "from events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime e "
    ...
    public List<EventInstance> getPendingEventsByActiveTs(int userId) {
    	Date date = new Date();
    	date.setSeconds(date.getSeconds() - 5);
        List<EventInstance> results = query(EVENT_SELECT_WITH_USER_DATA
                + "where e.activeTs<? and e.activeTs>? and ue.userId=? and e.ackTs is null order by e.activeTs desc", new Object[] { System.currentTimeMillis(),date.getTime(),userId },
                new UserEventInstanceRowMapper(), MAX_PENDING_EVENTS);
        attachRelationalInfo(results);
        return results;
    }
    ...
    public int purgeEventsBefore(final long time) {
        ...
        int count = getTransactionTemplate().execute(new GenericTransactionCallback<Integer>() {
            ...
            public Integer doInTransaction(TransactionStatus status) {
                int count = ejt2.update("delete from events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime " + "where activeTs<? " + "  and ackTs is not null "
    ...
    public int getEventCount() {
        return ejt.queryForInt("select count(*) from events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime");
    }
    ...
    private List<EventHandlerVO> getEventHandlers(int typeId, int ref1, int ref2) {
        return query(EVENT_HANDLER_SELECT + "where mangocode=? and eventTypeId=? and eventTypeRef1=? "
                + "  and (eventTypeRef2=? or eventTypeRef2=0)", new Object[] { Common.getEnvironmentProfile().getString("db.mangocode"),typeId, ref1, ref2 },
                new EventHandlerRowMapper());
    }
    //new code 
    public List<EventHandlerVO> getEventHandlerByalias(String alias) {
        return query(EVENT_HANDLER_SELECT + "where alias like '%"+alias+"%'", new Object[] {  }, new EventHandlerRowMapper());
    }
    ...
    public EventHandlerVO getEventHandler(String xid) {
        return queryForObject(EVENT_HANDLER_SELECT + "where xid=? and mangocode=?", new Object[] { xid,Common.getEnvironmentProfile().getString("db.mangocode")}, new EventHandlerRowMapper(),
                null);
    }
    ...
    //new code 
    public EventHandlerVO saveEventHandlersn(final EventTypeVO type, final EventHandlerVO handler) {
        if (type == null)
            return saveEventHandlersn(0, 0, 0, handler);
        return saveEventHandlersn(type.getTypeId(), type.getTypeRef1(), type.getTypeRef2(), handler);
    }

    private EventHandlerVO saveEventHandlersn(final int typeId, final int typeRef1, final int typeRef2,
            final EventHandlerVO handler) {
        getTransactionTemplate().execute(new TransactionCallbackWithoutResult() {
            @Override
            protected void doInTransactionWithoutResult(TransactionStatus status) {

                    updateEventHandlersn(handler);
            }
        });
    ...
    void insertEventHandler(int typeId, int typeRef1, int typeRef2, EventHandlerVO handler) {
        ...
        handler.setId(doInsert(
                "insert into eventHandlers (xid, alias, eventTypeId, eventTypeRef1, eventTypeRef2, data,mangocode) values (?,?,?,?,?,?,?)",
                new Object[] { handler.getXid(), handler.getAlias(), typeId, typeRef1, typeRef2,
                        SerializationHelper.writeObject(handler),mangocode }, new int[] { Types.VARCHAR, Types.VARCHAR,
                        Types.INTEGER, Types.INTEGER, Types.INTEGER, Types.BLOB,Types.VARCHAR }));
    ...
    void updateEventHandler(EventHandlerVO handler) {
        ...
        String mangocode=Common.getEnvironmentProfile().getString("db.mangocode");
        ejt.update("update eventHandlers set xid=?, alias=?, data=?,mangocode=? where id=?", new Object[] { handler.getXid(),
                handler.getAlias(), SerializationHelper.writeObject(handler),mangocode, handler.getId() }, new int[] {
                Types.VARCHAR, Types.VARCHAR, Types.BLOB,Types.VARCHAR, Types.INTEGER });
        AuditEventType.raiseChangedEvent(AuditEventType.TYPE_EVENT_HANDLER, old, handler);
    }
    //new code dao
    void updateEventHandlersn(EventHandlerVO handler) {
        EventHandlerVO old = getEventHandler(handler.getId());
        String mangocode=Common.getEnvironmentProfile().getString("db.mangocode");
        ejt.update("update eventHandlers  data=?,mangocode=? where id=?", new Object[] {SerializationHelper.writeObject(handler),mangocode, handler.getId() }, new int[] {
...
//
    ...
    // /
    //
    private static final String SILENCED_SELECT = "select ue.silenced " + "from events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_realtime e "
            + "  join userEvents ue on e.id=ue.eventId " + "where e.id=? " + "  and ue.userId=? "
            + "  and e.ackTs is null";

    public boolean toggleSilence(String eventId, int userId) {
    ...
    public int getHighestUnsilencedAlarmLevel(int userId) {
        return ejt.queryForInt("select max(e.alarmLevel) from userEvents u " + "  join events_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime e on u.eventId=e.id "
