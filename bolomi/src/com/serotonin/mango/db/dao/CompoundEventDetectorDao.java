...
public class CompoundEventDetectorDao extends BaseDao {
    ...
    public List<CompoundEventDetectorVO> getCompoundEventDetectors() {
        return query(COMPOUND_EVENT_DETECTOR_SELECT + " where mangocode='"+Common.getEnvironmentProfile().getString("db.mangocode")+"' order by name", new CompoundEventDetectorRowMapper());
    }
    public List<CompoundEventDetectorVO> getCompoundEventDetectorsByTj(String name) {
        return query(COMPOUND_EVENT_DETECTOR_SELECT + " where name like '%"+name+"%' and mangocode='"+Common.getEnvironmentProfile().getString("db.mangocode")+"' order by name", new CompoundEventDetectorRowMapper());
    }
    ...
    public CompoundEventDetectorVO getCompoundEventDetector(String xid) {
        return queryForObject(COMPOUND_EVENT_DETECTOR_SELECT + " where mangocode=? and xid=?", new Object[] { Common.getEnvironmentProfile().getString("db.mangocode"),xid },
    ...
    private static final String COMPOUND_EVENT_DETECTOR_INSERT = "insert into compoundEventDetectors (xid, name, alarmLevel, returnToNormal, disabled, conditionText,mangocode) "
            + "values (?,?,?,?,?,?,?)";
    ...
    private void insertCompoundEventDetector(CompoundEventDetectorVO ced) {
        ...
        int id = doInsert(COMPOUND_EVENT_DETECTOR_INSERT, new Object[] { ced.getXid(), ced.getName(),
                ...
                ced.getCondition(),mangocode });
        ced.setId(id);
        AuditEventType.raiseAddedEvent(AuditEventType.TYPE_COMPOUND_EVENT_DETECTOR, ced);
    }

    private static final String COMPOUND_EVENT_DETECTOR_UPDATE = "update compoundEventDetectors set xid=?, name=?, alarmLevel=?, returnToNormal=?, disabled=?, conditionText=?,mangocode=? "
            + "where id=?";
    ...
    private void updateCompoundEventDetector(CompoundEventDetectorVO ced) {
        ...
        ejt.update(COMPOUND_EVENT_DETECTOR_UPDATE, new Object[] { ced.getXid(), ced.getName(), ced.getAlarmLevel(),
                boolToChar(ced.isReturnToNormal()), boolToChar(ced.isDisabled()), ced.getCondition(), mangocode,ced.getId() });
        ...
        if (ced != null) {
            getTransactionTemplate().execute(new TransactionCallbackWithoutResult() {
                ...
                protected void doInTransactionWithoutResult(TransactionStatus status) {
                    ejt2.update("delete from eventHandlers where mangocode="+Common.getEnvironmentProfile().getString("db.mangocode")+" and eventTypeId=" + EventType.EventSources.COMPOUND
