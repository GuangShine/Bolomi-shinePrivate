...
public class PublisherDao extends BaseDao {
    ...
    public void savePublisher(final PublisherVO<? extends PublishedPointVO> vo) {
        ...
        if (vo.getId() == Common.NEW_ID){
            vo.setId(doInsert("insert into publishers (xid, data,mangocode) values (?,?,?)", new Object[] { vo.getXid(),
                    SerializationHelper.writeObjectToArray(vo),mangocode }, new int[] { Types.VARCHAR, Types.BLOB,Types.VARCHAR }));
         
         
        }
        else
        { ejt.update("update publishers set xid=?, data=?,mangocode=? where id=?", new Object[] { vo.getXid(),
                    SerializationHelper.writeObject(vo), mangocode,vo.getId() }, new int[] { Types.VARCHAR, Types.BLOB,Types.VARCHAR,
                    Types.INTEGER });
        }
    }
    public void savePublishDataPointLK(String dpXid,String pubXid,String dpName)
    {
    	ejt.update("Insert into publishdatapointLK (datapointXid,publishXid,datapointName) values(?,?,?)", new Object[] {dpXid,
    			  pubXid,dpName}, new int[] { Types.VARCHAR, Types.VARCHAR,Types.VARCHAR });
    	
    }
    public void deletePublishDataPointLK(String pubXid)
    {
    	ejt.update("delete from publishdatapointLK where PublishXid=?", new Object[] {pubXid}, new int[] { Types.VARCHAR});
    }
    ...
    public void deletePublisher(final int publisherId) {
        ...
        getTransactionTemplate().execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                ejt2.update("delete from eventHandlers where mangocode="+Common.getEnvironmentProfile().getString("db.mangocode")+" and eventTypeId=" + EventType.EventSources.PUBLISHER
