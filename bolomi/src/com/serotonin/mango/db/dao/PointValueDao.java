...
public class PointValueDao extends BaseDao {
    ...
    private static final String POINT_VALUE_INSERT_START = "insert into pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime (dataPointId, dataType, pointValue, ts) values ";
    ...
    long savePointValueImpl(final int pointId, final PointValueTime pointValue, final SetPointSource source,
        ...
        double dvalue = 0;
        String svalue = null;
        LOG.debug("DT--------------------"+dataType+"DHSDRT=--===---===---"+value.hasDoubleRepresentation()+"-------VALUE:"+value.getObjectValue()+"---------double value:"+dvalue);
        ...
        long id;
        LOG.debug("DOUBLE-------------------VALUE:"+dvalue+"OBJECT ------------VALUE:"+value.getObjectValue()+"string value================"+svalue);
        try {
    ...
    long savePointValue(final int pointId, final int dataType, double dvalue, final long time, final String svalue,
        ...
        if (async) {
        	...
        	String id=Common.getEnvironmentProfile().getString("db.mangocode")+pointId+date.get(Calendar.YEAR)+(date.get(Calendar.MONTH)+1)+date.get(Calendar.DAY_OF_MONTH)+date.get(Calendar.HOUR_OF_DAY)+date.get(Calendar.MINUTE)+date.get(Calendar.SECOND)+date.get(Calendar.MILLISECOND);
    ...
    private long savePointValueImpl(int pointId, int dataType, double dvalue, long time, String svalue,
    	...
    	/* Calendar date=Calendar.getInstance();*/
    	/*String valueid=Common.getEnvironmentProfile().getString("db.mangocode")+pointId+new Date().getTime()+System.currentTimeMillis();
        long id =	ejt.update(POINT_VALUE_INSERT, new Object[] { valueid,pointId, dataType, dvalue, time });*/
        		/*doInsertLong(POINT_VALUE_INSERT, new Object[] { valueid,pointId, dataType, dvalue, time });*/
        ...
        if (svalue != null || source != null) {
            ...
            ejt.update(POINT_VALUE_ANNOTATION_INSERT,
                    new Object[] { id, shortString, longString, sourceType, sourceId }, new int[] { Types.VARCHAR,
    ...
    private static final String POINT_VALUE_SELECT = "select pv.dataType, pv.pointValue, pva.textPointValueShort, pva.textPointValueLong, pv.ts, pva.sourceType, "
            + "  pva.sourceId "
            + "from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime pv "
    ...
    public PointValueTime getLatestPointValue(int dataPointId) {
        long maxTs = ejt.queryForLong("select max(ts) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=?",
    ...
    public PointValueTime getPointValueBefore(int dataPointId, long time) {
        Long valueTime = queryForObject("select max(ts) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=? and ts<?", new Object[] {
    ...
    private void updateAnnotations(String sql, Map<Integer, List<AnnotatedPointValueTime>> idMap) {
        ...
        List<IntStringPair> data = query(sql + "(" + createDelimitedList(idMap.keySet(), ",", null) + ")",
    ...
    public long deletePointValuesBefore(int dataPointId, long time) {
        return deletePointValues("delete from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=? and ts<?", new Object[] { dataPointId,
                time });
    }

    public long deletePointValues(int dataPointId) {
        return deletePointValues("delete from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=?", new Object[] { dataPointId });
    }

    public long deleteAllPointData() {
        return deletePointValues("delete from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where mangocode", null);
    }

    public long deletePointValuesWithMismatchedType(int dataPointId, int dataType) {
        return deletePointValues("delete from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=? and dataType<>?", new Object[] {
    ...
    public long dateRangeCount(int dataPointId, long from, long to) {
        return ejt.queryForLong("select count(*) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=? and ts>=? and ts<=?",
                new Object[] { dataPointId, from, to });
    }

    public long getInceptionDate(int dataPointId) {
        return ejt
                .queryForLong("select min(ts) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId=?", new Object[] { dataPointId }, -1);
    }

    public long getStartTime(List<Integer> dataPointIds) {
        if (dataPointIds.isEmpty())
            return -1;
        return ejt.queryForLong("select min(ts) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId in ("
    ...
    public long getEndTime(List<Integer> dataPointIds) {
        if (dataPointIds.isEmpty())
            return -1;
        return ejt.queryForLong("select max(ts) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId in ("
                + createDelimitedList(dataPointIds, ",", null) + ")");
    }
    ...
    public LongPair getStartAndEndTime(List<Integer> dataPointIds) {
        ...
        return queryForObject(
                "select min(ts), max(ts) from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataPointId in ("
    ...
    public List<Long> getFiledataIds() {
        return queryForList("select distinct id from ( " //
                + "  select id as id from pointValues_"+Common.getEnvironmentProfile().getString("db.mangocode")+"_RealTime where dataType="
                + DataTypes.IMAGE
                + "  union"
    ...
    class BatchWriteBehindEntry {
      ...
      /*  public BatchWriteBehindEntry(String id,int pointId, int dataType, double dvalue, long time) {
        	this.id=id;
        	this.pointId = pointId;
