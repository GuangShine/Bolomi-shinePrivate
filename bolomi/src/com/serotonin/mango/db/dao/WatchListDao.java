...
public class WatchListDao extends BaseDao {
    ...
    public List<WatchList> getWatchLists(final int userId) {
        ...
        return query("select id, xid, userId, name from watchLists " //
                + "where userId=? and mangocode=? or id in (select watchListId from watchListUsers where userId=?) " //
                + "order by name", new Object[] { userId,mangocode, userId }, new WatchListRowMapper());
    }
    ...
    public List<WatchList> getWatchLists() {
        return query("select id, xid, userId, name from watchLists where mangocode='"+Common.getEnvironmentProfile().getString("db.mangocode")+"'", new WatchListRowMapper());
    }
    ...
    public WatchList getWatchList(String xid) {
        return queryForObject("select id, xid, userId, name from watchLists where xid=? and mangocode="+Common.getEnvironmentProfile().getString("db.mangocode"), new Object[] { xid },
    ...
    public WatchList createNewWatchList(WatchList watchList, int userId) {
        ...
        String mangocode=Common.getEnvironmentProfile().getString("db.mangocode");
        watchList.setId(doInsert("insert into watchLists (xid, userId, name,mangocode) values (?,?,?,?)",
                new Object[] { watchList.getXid(), userId, watchList.getName(),mangocode }));
        return watchList;
    }
        ...
        getTransactionTemplate().execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                ...
                if (watchList.getId() == Common.NEW_ID)
                    watchList.setId(doInsert("insert into watchLists (xid, name, userId,mangocode) values (?,?,?,?)", new Object[] {
                            watchList.getXid(), watchList.getName(), watchList.getUserId(),mangocode }));
                else
                    ejt2.update("update watchLists set xid=?, name=?,mangocode=? where id=?", new Object[] { watchList.getXid(),
                            watchList.getName(),mangocode, watchList.getId() });
