...
LOG.debug("jinlaile-----------------------------------------");
    	if (logValue) {
    		LOG.debug("ZCJL-----------------------------------------");
            if (async){
            	//Log.debug("PVCA1111111111111111111111111111111111="+pvt.getValue()+"--------------------"+DataTypes.getDataType(pvt.getValue()));
            	LOG.debug("1111111111111111111111111111111id="+dataPointId);
            	/*LOG.debug("1111111111111111111111111111111source"+source.getSetPointSourceId());*/
                dao.savePointValueAsync(dataPointId, pvt, source);}
            else{
            	LOG.debug("PVCA2222222222222222222222222222222222");
