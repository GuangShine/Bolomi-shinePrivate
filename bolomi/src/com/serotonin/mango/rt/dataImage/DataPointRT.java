...
public class DataPointRT implements IDataPoint, ILifecycle, TimeoutClient {
    ...
    private void savePointValue(PointValueTime newValue, SetPointSource source, boolean async) {
        ...
        case DataPointVO.LoggingTypes.ON_CHANGE:
            if (pointValue == null){
            	LOG.debug("CASSSSSSSSSSSSSSSSSSSSSS----1");
                logValue = true;}
            else if (backdated)
                // Backdated. Ignore it
                ...
                if (newValue.getValue() instanceof NumericValue) {
                	LOG.debug("CASSSSSSSSSSSSSSSSSSSSSS----3");
                    // Get the new double
                    double newd = newValue.getDoubleValue();
            ...
            saveValue = logValue;
            LOG.debug("CASSSSSSSSSSSSSSSSSSSSSS----1VALUE"+saveValue);
            break;
        ...
        MangoValue vszzz=newValue.getValue();
        LOG.debug("~~~~~~~~~~~~~~~~~~~~-R-----------"+vszzz.getObjectValue());
