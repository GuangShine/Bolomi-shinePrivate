...
import com.xytl.mangosim.utils.jssc.SerialParameters;
import com.xytl.mangosim.utils.jssc.SerialPortException;
import com.xytl.mangosim.utils.jssc.SerialPortProxy;
import com.xytl.mangosim.utils.jssc.SerialPortProxyEvent;
import com.xytl.mangosim.utils.jssc.SerialPortProxyEventListener;
import com.xytl.mangosim.utils.jssc.SerialUtils;
import com.xytl.mangosim.utils.jssc.StreamUtils;
...
public class SerialDataSourceRT extends PollingDataSource implements SerialPortProxyEventListener{
	...
	public boolean connect () throws Exception{
        ...
        if ( SerialUtils.portOwned(vo.getCommPortId()) ){
			raiseEvent(DATA_SOURCE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.portInUse",vo.getCommPortId()));
			return false;
        }else{
        	try{
    			...
    			raiseEvent(DATA_SOURCE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.portError",vo.getCommPortId(),e.getLocalizedMessage()));
    			return false;
            }
        }
      /*  if ( vo.getCommPortId()== EncodingType.ASCII.toString()){
			raiseEvent(DATA_SOURCE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.portInUse",vo.getCommPortId()));
			return false;
        }else{
        	try{
                this.port = SerialUtils.openSerialPort(params);
                this.port.addEventListener((SerialPortProxyEventListener) this);
                return true;
    ...
    public void initialize() {
    	...
    	try{
			...
			raiseEvent(DATA_SOURCE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.connectFailed",msg));
    ...
    public void terminate() {
        ...
        if(this.port != null)
			try {
				...
				raiseEvent(DATA_SOURCE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.portError",this.port.getParameters().getCommPortId(),e.getLocalizedMessage()));
	...
	public void setPointValue(DataPointRT dataPoint, PointValueTime valueTime,
		...
		//Are we connected?
		if(this.port == null){
			raiseEvent(POINT_WRITE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.writeFailedPortNotSetup"));
			return;
		}
		...
		try {
	        ...
	        if(this.vo.isHex()){
	        	...
	        	try{
	    			...
	    			raiseEvent(POINT_WRITE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.notHex"));
	    			return;
	        	}
	        }else{
	        	//Pin the terminator on the end
				String messageTerminator = (this.vo).getMessageTerminator();
	      ...
	      /*  if(this.vo.isLogIO())
			...
			raiseEvent(POINT_WRITE_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.writeFailed",e.getMessage()));
		}
...
//			if(this.port == null){
//				raiseEvent(POINT_READ_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.readFailedPortNotSetup"));
//				return;
//			}
//			
...
//	            		buffer.popAll();
//	    				raiseEvent(POINT_READ_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.readFailed", "Max message size reached!"));
//	    				return; //Give up
//	            	}
//	            	buffer.push(data);
//	            }
...
//				raiseEvent(POINT_READ_EXCEPTION_EVENT, System.currentTimeMillis(), true, new LocalizableMessage("event.serial.readFailed",e.getMessage()));
//				
//			}
//		}//End synch
//	}
	...
	private void matchPointValues(String msg, String messageRegex, int pointIdentifierIndex) {
		...
		if(!this.dataPoints.isEmpty()){
            ...
            if(messageMatcher.matches()){
            		...
            		if(plVo.getPointIdentifier().equals(pointIdentifier)){
                    	...
                    	if(pointValueMatcher.matches()){
                        	...
                        	if(this.vo.isHex()){
                    			...
                    			dataValue = MangoValue.stringToValue(value, plVo.getDataTypeId());
                    		}
                        	
                        	if(dataValue != null){
                        		LOG.debug("-------------------TEST-----------"+dataValue);
	                        	PointValueTime newValue = new PointValueTime(dataValue,Common.timer.currentTimeMillis());
	                        	LOG.debug("-------------------TEST----------type:"+newValue.getValue().getDataType()+"-----------------value:"+newValue.getValue());
	                        	dp.updatePointValue(newValue);
                        		if(LOG.isDebugEnabled())
                        			LOG.debug("Saving value: " + newValue.toString());
                        	}else{
                            	raiseEvent(POINT_READ_PATTERN_MISMATCH_EVENT,System.currentTimeMillis(),true,new LocalizableMessage("event.serial.invalidValue",dp.getVO().getXid()));
                            	matcherFailed = true;
                        	}
            	...
            	raiseEvent(POINT_READ_PATTERN_MISMATCH_EVENT,System.currentTimeMillis(),true,new LocalizableMessage("event.serial.patternMismatch",vo.getMessageRegex(),msg));
            	matcherFailed = true;
            }
