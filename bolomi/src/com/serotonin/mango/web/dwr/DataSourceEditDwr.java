...
import com.serotonin.mango.db.dao.KeyDao;
...
public class DataSourceEditDwr extends DataSourceListDwr {
    ...
    public DwrResponseI18n editInit() {
        ...
        List list=defaultAll();
        if(list==null){response.addData("errorMes", "no data");}else{
        List<DataPointVO> listdpd=(List<DataPointVO>) list.get(0);
        List<PageVO> listpvo=(List<PageVO>) list.get(1);
        response.addData("points", list.get(0));
        response.addData("InfoCount", listpvo.get(0).getInfoCount());
        response.addData("NowPage", listpvo.get(0).getNowPage());
        response.addData("PageSize", listpvo.get(0).getPageSize());
        response.addData("PageCount", listpvo.get(0).getPageCount());      
       /* response.addData("points", getPoints());*/
        response.addData("alarms", getAlarms());
        response.addData("errorMes", null);
        }
        return response;     
    ...
    public List<PageVO> getPages() {
    	List list=getPointsByPage();
    
    	return (List<PageVO>)list.get(1);
    }
    ...
    //new code
    @MethodFilter
    public String getkey_lk(String xid) {
    	KeyDao ktd=new KeyDao();
    	 List<Key_lk> listkey_lk=ktd.getKeyTypeListByRefCode(xid);
     	String strlist="";
	     for(int i=0;i<listkey_lk.size();i++)
	     {
	     	strlist=strlist+listkey_lk.get(i).getKey_Code();
	     }
	     return strlist;
    }
    
    @MethodFilter
    public int getPointsCount(int datasourceid) {
    	  User user = Common.getUser();

          DataSourceVO<?> ds = user.getEditDataSource();

    	return new DataPointDao().getDataPointsCount(ds.getId());
    }
    @MethodFilter
    public List<DataPointVO> getPoints() {
        ...
        /*List pointAndPage=new DataPointDao().getDataPointsByPage(ds.getId(), DataPointNameComparator.instance);
        List<DataPointVO> points=(List<DataPointVO>) pointAndPage.get(0);*/
        return points;
    }
    	//NEW CODE
    public DwrResponseI18n selectInfo(int pageSizeSize)
   	{
   	 DwrResponseI18n response = new DwrResponseI18n();
	  User user = Common.getUser();
       if (user == null)
       ...
       if (ds.getId() == Common.NEW_ID)
           return null;
       List pointAndPage=new DataPointDao().selectInfo(ds.getId(), DataPointNameComparator.instance, pageSizeSize);
       List<DataPointVO> listdpd=(List<DataPointVO>) pointAndPage.get(0);
       List<PageVO> listpvo=(List<PageVO>) pointAndPage.get(1);
       response.addData("points", pointAndPage.get(0));
       response.addData("InfoCount", listpvo.get(0).getInfoCount());
       response.addData("NowPage", listpvo.get(0).getNowPage());
       response.addData("PageSize", listpvo.get(0).getPageSize());
       response.addData("PageCount", listpvo.get(0).getPageCount());
    ...
    public DwrResponseI18n goInfo(int nowpager,int pageContent,int pageSizeSize)
   	{
    	 DwrResponseI18n response = new DwrResponseI18n();
		  User user = Common.getUser();
	        if (user == null)
	        ...
	        List pointAndPage=new DataPointDao().goInfo(ds.getId(), DataPointNameComparator.instance, nowpager, pageContent, pageSizeSize);
    ...
    public DwrResponseI18n upInfo(int nowpager,int pageContent,int pageSizeSize)
   	{
    	 DwrResponseI18n response = new DwrResponseI18n();
		  User user = Common.getUser();
	        if (user == null)
	        ...
	        List pointAndPage=new DataPointDao().upInfo(ds.getId(), DataPointNameComparator.instance, nowpager, pageContent, pageSizeSize);
    ...
    public DwrResponseI18n nextInfo(int nowpager,int pageContent,int pageSizeSize)
	{
    	 DwrResponseI18n response = new DwrResponseI18n();
		  User user = Common.getUser();
	        if (user == null)
	        ...
	        List pointAndPage=new DataPointDao().nextInfo(ds.getId(), DataPointNameComparator.instance, nowpager, pageContent, pageSizeSize);
	...
	public List defaultAll()
	{
	      ...
	      /* List<DataPointVO> points = new DataPointDao().getDataPoints(ds.getId(), DataPointNameComparator.instance);
	       */
	       List pointAndPage=new DataPointDao().defaultInfo(ds.getId(), DataPointNameComparator.instance);
	        return pointAndPage;
	        /*return points;*/
	}
   /* public List PageUp(int nowpager,int pageContent,int pageSize)  
    {  
    	 User user = Common.getUser();
         if (user == null)
             return null;
         ...
         if (ds.getId() == Common.NEW_ID)
             return null;
         List pointAndPage=new DataPointDao().PageUp(ds.getId(), DataPointNameComparator.instance, nowpager, pageContent, pageSize);
         List<DataPointVO> points=(List<DataPointVO>) pointAndPage.get(0);
         return points;

    }*/
    @MethodFilter
    public List<DataPointVO> getPointsByPage() {
        User user = Common.getUser();
        if (user == null)
            return null;
    ...
    @MethodFilter
    public DataPointVO copyGetPoint(int pointId) {
        return copyGetPoint(pointId, null);
    }
    private DataPointVO  copyGetPoint(int pointId, DataPointDefaulter defaulter) {
        DataSourceVO<?> ds = Common.getUser().getEditDataSource();
    ...
    @MethodFilter
    public DwrResponseI18n saveModbusPointLocator(int id, String xid, String name,List dpkey, ModbusPointLocatorVO locator) {
        this.Savekey_dp(dpkey, xid);
    	return validatePoint(id, xid, name, locator, null);
    }
    ...
    public DwrResponseI18n saveModbusIpDataSource(String name, String xid, int updatePeriods, int updatePeriodType,
            ...
            String host, int port, boolean encapsulated,List dskey) {
        ModbusIpDataSourceVO ds = (ModbusIpDataSourceVO) Common.getUser().getEditDataSource();
       /* KeyDao kd=new KeyDao();
        List<Key_lk> listlk=new ArrayList<Key_lk>();
        for(int i=0;i<dskey.size();i++)
        {
        	Date date=new Date();
        	Key_lk key_lk=new Key_lk();
        	key_lk.setKey_Code(dskey.get(i).toString());
        	key_lk.setKeyType(1);
        	key_lk.setRef_Code(xid);  
        	key_lk.setKey_lk_cTime(date.toString());
        	if(kd.SearchByCodeIsE(key_lk)==1)
        	{
        		
        		key_lk=kd.SearchByCode(key_lk);
        		key_lk.setKey_lk_uTime(date.toString());
        	}
        	listlk.add(key_lk);
        }
        for(int i=0;i<listlk.size();i++)
        {        	
        		kd.Savekey(listlk.get(i));
        }*/
        this.Savekey_ds(dskey, xid);
        ds.setXid(xid);
        ds.setName(name);
    ...
    public DwrResponseI18n saveBACnetIpDataSource(String name, String xid, int updatePeriods, int updatePeriodType,
            ...
            int maxReadMultipleReferencesNonsegmented,List dskey) {
    ...
    @MethodFilter
    public DwrResponseI18n saveBACnetIPPointLocator(int id, String xid, String name,List dpkey, BACnetIPPointLocatorVO locator) {
    	this.Savekey_dp(dpkey, xid);
    	return validatePoint(id, xid, name, locator, null);
    }
    ...
    //new code
    @MethodFilter
    public DwrResponseI18n saveSerialDataSource( String name, String xid,String commPortId, int baudRate, int flowControlIn,
    ...
    @MethodFilter
    public DwrResponseI18n savePointLocator(int id, String xid, String name,SerialPointLocatorVO locator) {
    	if(locator.getPointIdentifier() == null)
    		locator.setPointIdentifier(new String()); //Sometimes we want to match an empty string
        return validatePoint(id, xid, name, locator, null);
    }
    @MethodFilter
    public DwrResponseI18n testString(String raw) {
    	DwrResponseI18n pr = new DwrResponseI18n();
    ...
    public void Savekey_ds(List dskey,String xid)
    {
    	 KeyDao kd=new KeyDao();
    ...
    public void Savekey_dp(List dpkey,String xid)
    {	
    	KeyDao kd=new KeyDao();
    	kd.deletekey_lk(xid);
    	...
    	if(dpkey.size()==0)
    	{
         ...
         for(int i=0;i<dpkey.size();i++)
         {
         	Date date=new Date();
         	Key_lk key_lk=new Key_lk();
         	key_lk.setKey_Code(dpkey.get(i).toString());
         	key_lk.setKeyType(2);
         	key_lk.setRef_Code(xid);  
         	key_lk.setKey_lk_cTime(date.toString());
         	if(kd.SearchByCodeIsE(key_lk)==1)
